...
int ata_probe( int id, int *nblocks, int *blocksize, char *name ) {
	...
	if(t == 0xff) {
		printk("ATA probing: nothing attached on drive %u", id);
	...
	if(ata_identify(id, ATA_COMMAND_IDENTIFY, cbuffer)) {
		...
		*blocksize = 2048;
		...
		printk("ATA probing: identify command failed on drive %u", id);
	...
	name[40] = 0;
	printk("ATA probing: saving info to drive list, found disk on drive %u: %f MB.", id, (float)(*nblocks)*(*blocksize)/1024/1024);
