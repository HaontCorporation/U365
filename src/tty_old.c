...
{
	//Setting up character sizes.
	chr_h = 16;
	chr_w = 8;

	if(kernel_mboot_info->framebuffer_type!=1) //We have text mode support... but it doesn't work.
	{
		tty_h = 24;
		tty_w = 80;
	}
	...
	{
		tty_h = vesa_fb_height/chr_h-1;
		tty_w = vesa_fb_width/chr_w+1;
	}
	tty_x = tty_y = 0;
	tty_color=TERM_DEFAULT_COLOR;
	uint8_t buf[tty_h * tty_w*2*2];
	tty_buf = (uint8_t*) buf;
...
void terminal_scroll()
{
	...
	memmove(vesa_fb_addr-1024*16*4, vesa_fb_addr, vesa_fb_width * vesa_fb_height * 4);/*
	for(int y = 0; y <= tty_h; y++)
	{
		for(int x = 0; x < tty_w; x++)
		{
